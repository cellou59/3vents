...
export function Schedule({events}: Props) {
    ...
    const filterEventsDetails = () => {
      if (Array.isArray(events) && events.length > 0) {
        const filteredEventsDetails = eventsDetails.map(day => {        
          const filteredPerformers = day.performers.filter(performer =>
            events.some(event => event.name === performer.name)
          );
        ...
        setDaysSchedule(filteredEventsDetails.slice(0,3));
        console.log('🚀 ~ file: Schedule.tsx:308 ~ filterEventsDetails ~ filteredEventsDetails:', filteredEventsDetails)
    ...
    return (
      ...
      >
        <Tab.List className="-mx-4 flex gap-x-4 gap-y-10 overflow-x-auto pb-4 pl-4 sm:mx-0 sm:flex-col sm:pb-0 sm:pl-0 sm:pr-8">
          {({ selectedIndex }) => (
            <>
              {daysSchedule && daysSchedule.map((day, dayIndex) => (
                <div
                  key={day.name}
                  className={clsx(
        ...
        <Tab.Panels>
          {daysSchedule  && daysSchedule.map((day) => (
            <Tab.Panel
              key={day.name}
  ...
  function TimeSlots({ day, className }: { day: Day; className?: string }) {
    return (
      ...
      >
        {day && day?.performers.map((timeSlot, timeSlotIndex) => (
          <li
            key={timeSlot.name}
            aria-label={`${timeSlot.price}  at ${timeSlot.time} - ${timeSlot.location}`}
          >
            {timeSlotIndex > 0 && (
